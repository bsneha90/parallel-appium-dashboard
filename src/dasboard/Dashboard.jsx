...
import { ItemGrid, StatsCard, Table } from "../components";
import {
  Devices,
  InfoOutline,
  Android,
...
class Dashboard extends Component {
  ...
  render() {
    ...
    const pieCharData = [
      { value: countOfPass, color: passColor },
      ...
      <div>
              ...
              <div className="PieChartWrapper">
                ...
                <div className="PieChartProgress">
                  ...
                  <List className="PieChartIndexList">
                    <ListItem>
                      <ListItemText
                        primary={`${(countOfPass * percentage).toFixed(2)}%`}
                      />
                    </ListItem>
                    <ListItem>
                      <ListItemText
                        primary={`${(countOfFail * percentage).toFixed(2)}%`}
                      />
                    </ListItem>
                    <ListItem>
                      <ListItemText
                        primary={`${(countOfUnknown * percentage).toFixed(2)}%`}
                      />
                    </ListItem>
                  </List>
                </div>
        ...
        <div className="DeviceInformationContainer">
          ...
          <Grid container>
            {devices.map(device => {
              const icon =
                device.getOS() === "Android" ? Android : AppleSvgIcon;
              const iconColor = device.getOS() === "Android" ? "green" : "gray";
              console.log(testOnDevices, "testOnDevices");
              const testsRunOnDevice =
                testOnDevices &&
                device.getUdid() in testOnDevices &&
                testOnDevices[device.getUdid()];
              const passCnt = testsRunOnDevice
                ? testsRunOnDevice.filter(t => t.testresult === "Pass").length
                : 0;
              const failCnt = testsRunOnDevice
                ? testsRunOnDevice.filter(t => t.testresult === "Fail").length
                : 0;
              const skipCnt = testsRunOnDevice
                ? testsRunOnDevice.filter(t => t.testresult === "Skip").length
                : 0;
              ...
              return (
                <ItemGrid xs={12} sm={6} md={4}>
                  <StatsCard
                    ...
                    description={`HostIp: ${device.getIp()}`}
                    statIconColor="info"
                    statText={
                      <StatText
                        passCnt={passCnt}
                        ...
                        skipCnt={skipCnt}
                        onClick={() =>
                          this.handleDeviceOnClick({
                            udid: device.getUdid(),
                            tests: testsRunOnDevice
                          })
                        }
                      />
                    }
  ...
  return (
    <button className="statbutton" onClick={onClick}>
      <div style={{ marginTop: "3px", display: "flex" }}>
        <div style={{ flex: 1, display: "flex" }}>
          <div style={{ flex: 1 }}>
            ...
            <CheckCircle
              ...
              className="statbuttonResultIcon"
            />
        ...
        <div style={{ flex: 1, alignItems: "flex-end", justifyItems: "end" }}>
          {" "}
          <ArrowForward style={{ float: "right" }} />{" "}
        </div>
      </div>
    </button>
  );
...
const StatText2 = props => {
  return (
    <Badge style={{ marginTop: "15px" }} badgeContent={4} color="primary">
      <CheckCircle style={{ color: passColor }} />
    </Badge>
  );
};
...
const StatText3 = props => {
  return (
    <Button variant="contained" className="statbutton">
      <CheckCircle style={{ color: passColor }} />
      <ArrowForward />
    </Button>
  );
